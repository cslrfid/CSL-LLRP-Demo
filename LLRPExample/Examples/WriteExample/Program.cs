...
namespace WriteExample
{
    class Program
    {
                ...
                for (int i = 0; i < msg.TagReportData.Length; i++)
                {
                    ...
                    if ((msg.TagReportData[i].AccessCommandOpSpecResult != null))
                    {
                        ...
                        if (msg.TagReportData[i].AccessCommandOpSpecResult.Count == 1)
                        {
                            ...
                            {
                                PARAM_C1G2WriteOpSpecResult read = (PARAM_C1G2WriteOpSpecResult)msg.TagReportData[i].AccessCommandOpSpecResult[0];
                                accessCount++;
                                data += " AccessData: ";
        ...
        static void usage()
        {
            Console.WriteLine("usage: WriteExample.exe <IP>");
            return;
        }
