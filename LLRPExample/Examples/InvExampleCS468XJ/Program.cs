...
namespace InvExample
{
    class Program
    {
        ...
        static void usage()
        {
            Console.WriteLine("usage: InvExample.exe <IP>");
            return;
        }
        ...
        static void Main(string[] args)
        {
            ...
            #region getReaderCapabilities
            {
                ...
                // is an CS463 reader (deviceManufacturerName == 51654)
                if ((dev_cap != null) &&
                    (dev_cap.DeviceManufacturerName == 51654))
                {
                }
                else
                {
            ...
            #region SetCustomDwellWithXML
            {
                Console.WriteLine("Adding Custom from XML file \n");

                Org.LLRP.LTK.LLRPV1.DataType.Message obj;
                ENUM_LLRP_MSG_TYPE msg_type;

                // read the XML from a file and validate its an Custom
                try
                {
                    FileStream fs = new FileStream("setCustomDwell.xml", FileMode.Open);
                    ...
                    if (obj == null || msg_type != ENUM_LLRP_MSG_TYPE.CUSTOM_MESSAGE)
                    {
                ...
                MSG_CUSTOM_MESSAGE msg = (MSG_CUSTOM_MESSAGE)obj;
                MSG_ERROR_MESSAGE msg_err;
                MSG_CUSTOM_MESSAGE rsp = reader.CUSTOM_MESSAGE(msg, out msg_err, 2000);
                if (msg_err != null)
                {
                    Console.WriteLine(msg_err.ToString());
                    reader.Close();
                ...
                {
                    Console.WriteLine("set CUSTOM_MESSAGE Command success\n");
                }
            }
            #endregion

            #region SetCustomInventoryModeWithXML
            {
                Console.WriteLine("Adding Custom from XML file \n");
                ...
                try
                {
                    FileStream fs = new FileStream("setCustomInventoryMode.xml", FileMode.Open);
                    StreamReader sr = new StreamReader(fs);
                    string s = sr.ReadToEnd();
                    fs.Close();
